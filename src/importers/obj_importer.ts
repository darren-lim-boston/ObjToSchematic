...
export class ObjImporter extends IImporter {
    ...
    constructor(rotate: boolean = false) {
        super();
        this.rotate = rotate;
    }
        ...
        },
        {
            ...
            delegate: (match: { [key: string]: string }) => {
                if (this.rotate) {
                    ...
                    const y = parseFloat(match.z); // SWITCHED THE Z AND Y, negated some
                    const z = parseFloat(match.y);
                    checkNaN(x, y, z);
                    this._vertices.push(new Vector3(z, y, x)); // rotate 90 degrees counter-clockwise
                } else {
                    const x = parseFloat(match.x);
    ...
    override parseFile(filePath: string) {
        // TODO: this does not work with linux?
    ...
    private _parseOBJ(pathStr: string) {
        if (!fs.existsSync(pathStr)) {
            throw new AppError(`Could not find ${pathStr}`);
        }

        if (pathStr.includes('_chunks_')) {
            // THIS WILL LOAD THEM BY CHUNKS
            const parent = pathStr.substring(0, pathStr.lastIndexOf('/') + 1);
            console.log('CHUNKING: parent is ' + parent);
            fs.readdirSync(parent).forEach((file) => {
                if (file.endsWith('obj') && file.includes('_chunks_')) {
                    file = parent + file;
                    console.log('CHUNKED: LOADING ' + file);
                    const fileContents = fs.readFileSync(file, 'utf8');
                    if (fileContents.includes('�')) {
                        throw new AppError(`Unrecognised character found, please encode <b>${file}</b> using UTF-8`);
                    }
            ...
            const fileContents = fs.readFileSync(pathStr, 'utf8');
            if (fileContents.includes('�')) {
                throw new AppError(`Unrecognised character found, please encode <b>${pathStr}</b> using UTF-8`);
            }
