...
export class BVHRayVoxeliser extends IVoxeliser {
    protected override _voxelise(mesh: Mesh, voxeliseParams: VoxeliseParams): VoxelMesh {
        ...
        console.log(`DESIRED HEIGHT IS ${Mesh.desiredHeight}`);
        ...
        useMesh.scaleMesh(voxeliseParams.desiredWidth);
        ...
        for (rayIndex = 0; rayIndex < rays.length; ++rayIndex) {
            ...
            if (diff >= 1000) {
                lastUpdate = new Date();
                LOG('RAY PROGRESS...' + rayIndex + '/' + rays.length + ' (' + ((rayIndex + 1) / rays.length * 100) + '%)');
            }
        }
