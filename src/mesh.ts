...
export class Mesh {
    ...
    public processMesh() {
        ...
        console.log("---reset loaded textures");
        this.resetLoadedTextures();
    }
    ...
    public scaleMesh(scaleFactor: number) {
        ...
        // Untranslate each triangle
        this.translateMesh(centre.mulScalar(scaleFactor));
    }
    ...
    public getMinPoint(): Vector3 {
        ...
        this._vertices.forEach((vertex) => {
            if(minPoint == undefined) {
                ...
                let voxelValue = vertex.x + vertex.z;
        ...
        if(minPoint == undefined) {
            return this.getBounds().min.copy();
        } else {
            return (minPoint as Vector3).copy();
        }
    }
    ...
    public resetLoadedTextures() {
        this._loadedTextures = {};
    }

    private _loadTexture(triIndex: number) {
        let loadedCount = 0;
        const tri = this._tris[triIndex];
        const material = this._materials[tri.material];
        if (material.type == MaterialType.textured) {
            if (!(tri.material in this._loadedTextures)) {
                console.log("LOADING: " + material.path);
    ...
    public sampleMaterial(materialName: string, uv: UV, textureFiltering: TextureFiltering, triIndex: number) {
        this._loadTexture(triIndex);
