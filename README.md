# What is this (City of Boston developer's note):

This a fork of the ObjToSchematic repository, as described below. Primarily, the changes were made in order to allow the schematics to be converted at a much more efficient rate than what was allowed in the original repository. Changes include allowing the input/output paths to be specified at runtime rather than before each build, and some axis optimizations. Additionally, a brief "chunking" mechanism is used to load very large files, which can be removed and fixed by pulling from the newest ObjToSchematic (which imports files as streams now).

## Possible future work

This can be used as-is, but better yet, some of the below changes can be made to make this tool even better:

- Update the tool to the newest version released by the original ObjToSchematic

- Clean-up the changes made so that the fork can be more useful by a generic audience

- This tool is using a single-threaded programming language (JS), which ends up reducing overall possible performance. If the tool was interfaced or completely rewritten in a different multi-threaded programming language, then long for-loop actions, such as the raycasting procedure, can be parallelized and completed at a much faster rate. This would probably take the longest to do out of all changes, but will lead to the most substantial benefits.

## How to use

Simply follow the instructions labeled in the boston-loader-convert repository to use this tool to convert .obj files to compatible .schematic files for Minecraft.
